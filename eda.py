...
def col_types(dataframe,  cat_th_ratio_upper=5,car_th_cat_th_lower=65, car_th_cat_th_upper=100):
    ...
    for col in dataframe.columns:
        col_list.append(col)
        diff_value_list.append(dataframe[col].nunique())
        per_list.append(dataframe[col].nunique() / dataframe.shape[0] * 100)
    print("\n")
    print(pd.DataFrame({'cat_col_name' :col_list,
                        'diff_value' : diff_value_list,
                        'diff_value_per' : per_list}).sort_values(by="diff_value_per",ascending=False))
